...
void videoInput::processPixels(unsigned char * src, unsigned char * dst, int width, int height, bool bRGB, bool bFlip){
	...
	if(!bRGB){
		...
		{
			int i;
			const uint8_t* a_ptr = (const uint8_t*)src;
		    const uint8_t* a_ptr2 = (const uint8_t*)(src + (height - 1) * widthInBytes);
		    uint8_t* b_ptr = dst + (height - 1) * widthInBytes;
		    uint8_t* b_ptr2 = dst;
			for(i = 0; i < height / 2; i++)
    		{
				memcpy(b_ptr, a_ptr, widthInBytes);
				memcpy(b_ptr2, a_ptr2, widthInBytes);
